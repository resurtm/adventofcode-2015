...
func (tc *testCase) solvePartTwo() int64 {
	tiles := make([][]int64, 1000)
	for i, _ := range tiles {
		tiles[i] = make([]int64, 1000)
	}
	...
	var brightness int64 = 0
	for i := 0; i < 1000; i++ {
		for j := 0; j < 1000; j++ {
			brightness += tiles[i][j]
		}
	}
	return brightness
}
