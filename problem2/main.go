...
func (tc *testCase) solvePartOne() int {
	...
	for _, m := range tc.input {
		dims, maxK, maxM := []int{}, -1, -1
		for k, ns := range strings.Split(m, "x") {
		...
		area += dims[(maxK+1)%3] * dims[(maxK+2)%3]
		area += dims[0] * dims[1] * 2
		area += dims[1] * dims[2] * 2
		area += dims[2] * dims[0] * 2
	}
	return area
}
