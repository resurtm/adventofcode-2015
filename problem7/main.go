...
func (tc *testCase) solvePartOne() uint16 {
	re := regexp.MustCompile(`^(NOT)?\s*(\w+|(\w+) (AND|OR|LSHIFT|RSHIFT) (\w+)) -> (\w+)$`)

	store := map[string]uint16{}

	checkStore := func(name string) {
		if _, ok := store[name]; !ok {
			store[name] = 0
		}
	}

	for _, expression := range tc.input {
		matches := re.FindAllStringSubmatch(expression, -1)

		if strings.Index(expression, "AND") != -1 || strings.Index(expression, "OR") != -1 {
			cell0, cell1 := matches[0][3], matches[0][5]
			cellOp, cellRes := matches[0][4], matches[0][6]
			checkStore(cell0)
			checkStore(cell1)
			checkStore(cellRes)
			if cellOp == "AND" {
				store[cellRes] = store[cell0] & store[cell1]
			} else if cellOp == "OR" {
				store[cellRes] = store[cell0] | store[cell1]
			}
		} else if strings.Index(expression, "LSHIFT") != -1 || strings.Index(expression, "RSHIFT") != -1 {
			cell0 := matches[0][3]
			cell1, _ := strconv.Atoi(matches[0][5])
			cellOp, cellRes := matches[0][4], matches[0][6]
			checkStore(cell0)
			checkStore(cellRes)
			if cellOp == "LSHIFT" {
				store[cellRes] = store[cell0] << uint16(cell1)
			} else if cellOp == "RSHIFT" {
				store[cellRes] = store[cell0] >> uint16(cell1)
			}
		} else if strings.Index(expression, "NOT") != -1 {
			cellFrom, cellTo := matches[0][2], matches[0][6]
			checkStore(cellFrom)
			checkStore(cellTo)
			store[cellTo] = ^store[cellFrom]
		} else {
			cellFrom, _ := strconv.Atoi(matches[0][2])
			cellTo := matches[0][6]
			checkStore(cellTo)
			store[cellTo] = uint16(cellFrom)
		}
	}
